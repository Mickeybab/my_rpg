...
** my_rpg_2017
** File description:
** Update the pos of the dead menu depending on the view
*/
...
static void change_pos_foreground(menu_t *menu, button_t **fore,
	sfVector2f pos)
{
	sfVector2f ref[NB_FOREGROUND] = POS_FOREGROUND;

	for (unsigned int i = 0; i < menu->number_foreground; i++) {
		fore[i]->position.x = (pos.x - (1920 / 2)) + ref[i].x;
		fore[i]->position.y = (pos.y - (1920 / 2)) + ref[i].y + 400;
...
static void change_pos_text(menu_t *menu, text_t **text, sfVector2f pos)
{
	...
	for (unsigned int i = 0; i < menu->number_text; i++) {
		text[i]->position.x = (pos.x - (1920 / 2)) + ref[i].x;
		text[i]->position.y = (pos.y - (1920 / 2)) + ref[i].y + 400;
		sfText_setPosition(text[i]->text, text[i]->position);
	}
}
