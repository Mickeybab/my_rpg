	...
	, sfVector2f *projected1, sfVector2f *projected2)
{
	for (int j = 0; j < obj1->point_count; j++) {
		for (int i = 0; i < obj1->point_count; i++)
			projected1[i] = project_point_on_axis
			(obj1->axis_slopes[j], obj1->points[i]);
		for (int i = 0; i < obj2->point_count; i++)
			projected2[i] = project_point_on_axis
			(obj1->axis_slopes[j], obj2->points[i]);
		if (detect_overlap(projected1, projected2
			, obj1->point_count, obj2->point_count) == 0)
			return (0);
	}
	return (1);
}
