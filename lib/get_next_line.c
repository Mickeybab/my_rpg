...
static void my_strcpy(char *destination, char *source, const int choice)
{
	unsigned int size = 0;
	unsigned int count = 0;
...
static char *my_re_alloc_string(char *old_string, const int to_add)
{
	...
	for (unsigned int i = 0; i < lenght_old_string + to_add; i++)
		new_string[i] = 0;
	my_strcpy(new_string, old_string, 2);
...
char *get_next_line(int fd)
{
	...
	if (!(line = malloc(sizeof(char) * size + size)) || !size)
		return (NULL);
	line[0] = '\0';
	buf[0] != '\n' ? my_strcpy(line, buf, 1) : 0;
	if (scan(buf)) {
		while ((check = read(fd, buf, size)) == size &&
		scan(buf) && line) {
			my_strcpy(line, buf, 1);
			line = my_re_alloc_string(line, size);
		}
		check > 0 && *buf != '\n' ? my_strcpy(line, buf, 1) : 1;
		check < size ? buf[check - 1] = '\0' : 0;
	}
	my_strcpy(buf, buf + my_line_lenght(buf, 1), 3);
